...
SimpleSwitch *global_simple_switch_ptr = nullptr;
...
SimpleSwitch::SimpleSwitch(bool enable_swap, port_t drop_port, uint32_t priority_queues)
  : Switch(), 
    drop_port(drop_port),
    ...
    egress_buffers(nb_egress_threads,
                   1024, std::move(EgressThreadMapper(nb_egress_threads)),
...
SimpleSwitch::~SimpleSwitch() {
  ...
  global_simple_switch_ptr = nullptr;
}
...
int SimpleSwitch::set_egress_priority_queue_depth(size_t port_id, size_t queue_id, size_t depth_pkts) {
  egress_buffers.set_capacity(port_id, queue_id, depth_pkts);
  return 0;
}

int SimpleSwitch::set_egress_priority_queue_rate(size_t port_id, size_t queue_id, uint64_t rate_pps) {
  return 0;
}
#endif
