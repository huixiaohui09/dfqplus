/*We have listed three distinct DFQ variants here, 
corresponding to the three ones specified in the ablation experiments. 
You may select one of these three DFQs at a time for conducting ablation experiments. 
By default, the DFQplus algorithm runs all other experiments. */
...
public class DFQQueue implements Queue<Packet> {
    ...
    private static final long TIME_PERIOD_NS = 1_000_000_0L; 
    ...
    public boolean offer(Packet packet) {
        ...
        try {
            ...
            if (currentTime - lastUpdateTime >= TIME_PERIOD_NS) {
            ...
            for (QueueInfo qInfo : queueList) {
                accumulatedBits = qInfo.remainingCapacity * packetSize + previousThresholdBits; 
                Long thresholdRank = findRankThreshold(accumulatedBits);
    ...
    private void manageQueues() {
        ...
        if (highestPriorityQueue != null && highestPriorityQueue.remainingCapacity > 0 && highestPriorityQueue.child == null) {
            splitQueue(highestPriorityQueue);
        }
    
        if (highestPriorityQueue != null && highestPriorityQueue.queue.isEmpty() && highestPriorityQueue.child != null) {
            mergeQueues(highestPriorityQueue);
        }
    }
    ...
    private void mergeQueues(QueueInfo parent) {
        QueueInfo child = parent.child;
        
        if (child == null) {
            return; 
        }
    
        child.remainingCapacity += parent.remainingCapacity;   
        queueList.remove(parent);
        parent.child = null;
    }
...
/*DFQ without head-to-tail reclamation, using only dynamic admission thresholds (denoted as DFQ (w/o R) */ 
/* 
package ch.ethz.systems.netbench.xpt.aifo.ports.DFQ;
...
public class DFQQueue implements Queue<Packet> {
    ...
    private long lastUpdateTime;
    private static final long TIME_PERIOD_NS = 1_000_000L;
    ...
    public boolean offer(Packet packet) {
        ...
        try {
            ...
            for (QueueInfo qInfo : queueList) {           
                if (!timeUpdated && currentTime - lastUpdateTime >= TIME_PERIOD_NS) {
                    lastUpdateTime = currentTime;
                    timeUpdated = true;
                }
    ...
    public Packet poll() {
        reentrantLock.lock();
        try {
            List<QueueInfo> tempList = new ArrayList<>(queueList);
            for (QueueInfo qInfo : tempList) {
                Packet packet = qInfo.queue.poll();
                if (packet != null) {
                    qInfo.remainingCapacity++;
                    return packet;
                } else {
                    queueList.remove(qInfo);
                    queueList.add(qInfo); 
                }
            }
            currentRound++;
